...
static bool _cfgDbAdd(CFG_DB_t* db, IO_LINE_t* line)
{
	...
	if (strncmp(line->line, "CFG-", 4) == 0)
	{
		...
		{
			log_warn("Could not parse value '%s' for item '%s' (type %s)!", valStr, item->name, ubloxcfg_typeStr(item->type));
			return false;
		}
	...
	else if ((strncmp(line->line, "UBX-", 4) == 0) || (strncmp(line->line, "NMEA-", 5) == 0)
	...
	// Port configuration
	else if ((strncmp(line->line, "UART1 ", 5) == 0) || (strncmp(line->line, "UART2 ", 5) == 0)
	         || (strncmp(line->line, "SPI ", 4) == 0) || (strncmp(line->line, "I2C ", 4) == 0)
	         || (strncmp(line->line, "USB ", 4) == 0))
	{
		...
		PORT_CFG_t portCfg[] = {
		    {.name       = "UART1",
		     .baudrateId = UBLOXCFG_CFG_UART1_BAUDRATE_ID,
		    ...
		    {.name       = "UART2",
		     .baudrateId = UBLOXCFG_CFG_UART2_BAUDRATE_ID,
