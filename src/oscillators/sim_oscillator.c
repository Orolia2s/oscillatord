...
static int          sim_oscillator_set_dac(struct oscillator* oscillator, uint32_t value)
{
...
static int sim_oscillator_get_dac(struct oscillator* oscillator, uint32_t* value)
{
	struct sim_oscillator* sim;
...
static int sim_oscillator_get_ctrl(struct oscillator* oscillator, struct oscillator_ctrl* ctrl)
{
...
static struct oscillator* sim_oscillator_new(struct devices_path* devices_path)
{
	...
	oscillator_factory_init(FACTORY_NAME, oscillator, FACTORY_NAME "-%d", sim_oscillator_index);

	log_debug("reading pts name");
	cret = fgets(sim->pps_pts, SIM_MAX_PTS_PATH_LEN, sim->simulator_process);
	if (cret == NULL)
	{
		ret = -EIO;
