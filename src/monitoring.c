...
static void json_add_clock_data(struct json_object* resp, struct monitoring* monitoring)
{
	...
	json_object_object_add(clock, "offset", json_object_new_int(monitoring->phase_error));
...
static void json_add_gnss_data(struct json_object* resp, struct monitoring* monitoring)
{
	...
	json_object_object_add(gnss, "fix", json_object_new_int(monitoring->fix));
	json_object_object_add(gnss, "fixOk", json_object_new_boolean(monitoring->fixOk));
	json_object_object_add(gnss, "antenna_power", json_object_new_int(monitoring->antenna_power));
	json_object_object_add(gnss, "antenna_status", json_object_new_int(monitoring->antenna_status));
	json_object_object_add(gnss, "lsChange", json_object_new_int(monitoring->lsChange));
	json_object_object_add(gnss, "leap_seconds", json_object_new_int(monitoring->leap_seconds));
	json_object_object_add(gnss, "satellites_count", json_object_new_int(monitoring->satellites_count));
	json_object_object_add(gnss, "survey_in_position_error", json_object_new_int(monitoring->survey_in_position_error));
...
struct monitoring* monitoring_init(const struct config* config, struct devices_path* devices_path)
{
	...
	if (port < 0)
	{
		log_error("Monitoring: Error %d fetching socket-port from config %s", port, config->path);
		return NULL;
	}
	...
	log_info("Monitoring: INITIALIZATION: Successfully started monitoring thread, listening on %s:%d", address, port);
	if (ret != 0)
	{
