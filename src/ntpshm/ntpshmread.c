...
struct shmTime* shm_get(const int unit, const bool create, const bool forall)
{
	 ...
	 */
	shmid = shmget((key_t)(NTPD_BASE + unit), sizeof(struct shmTime), (create ? IPC_CREAT : 0) | (forall ? 0666 : 0600));
	if (shmid == -1)
	{ /* error */
		if (2 != errno)
		{
			...
			(void)fprintf(stderr, "WARNING: could not open SHM(%d): %s(%d)", unit, strerror(errno), errno);
		}
		return NULL;
	}
...
enum segstat_t ntp_read(struct shmTime* shm_in, struct shm_stat_t* shm_stat, const bool consume)
{
	...
	case 0:
		...
		*/
		if (((cns_new - (unsigned)shm_stat->tvt.tv_nsec) < 1000) && ((rns_new - (unsigned)shm_stat->tvr.tv_nsec) < 1000))
		{
