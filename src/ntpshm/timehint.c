...
static volatile struct shmTime* getShmTime(struct gps_context_t* context, int unit)
{
	...
	if (shmid == -1)
	{
		log_error("NTP: shmget(%ld, %zd, %o) fail: %s", (long int)(NTPD_BASE + unit), sizeof(struct shmTime), (int)perms, strerror(errno));
		return NULL;
	}
	p = (struct shmTime*)shmat(shmid, 0, 0);
	if ((int)(long)p == -1)
	{
		log_error("NTP: shmat failed: %s", strerror(errno));
		return NULL;
	}
	log_trace("NTP: shmat(%d,0,0) succeeded, segment %d", shmid, unit);
	return p;
}
...
static char* report_hook(volatile struct pps_thread_t* pps_thread, struct timedelta_t* td)
...
void ntpshm_link_activate(struct gps_device_t* session)
{
	...
	if (session->sourcetype == source_usb || session->sourcetype == source_rs232 || session->sourcetype == source_pps)
	{
