...
static int get_edge_tiocmiwait(volatile struct pps_thread_t*, struct timespec*, int*, volatile struct timedelta_t*);
#endif /* TIOCMIWAIT */
...
#if defined(HAVE_SYS_TIMEPPS_H)
static int get_edge_rfc2783(struct inner_context_t*, struct timespec*, int*, struct timespec*, int*, volatile struct timedelta_t*);
#endif /* defined(HAVE_SYS_TIMEPPS_H) */
...
static void thread_lock(volatile struct pps_thread_t* pps_thread)
{
	...
	if (0 != pthread_err)
	{
		...
		pps_thread->log_hook(pps_thread, THREAD_ERROR, "PPS:%s pthread_mutex_lock() : %s", pps_thread->devicename, errbuf);
	}
}
...
static void thread_unlock(volatile struct pps_thread_t* pps_thread)
{
	...
	if (0 != pthread_err)
	{
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		pps_thread->log_hook(pps_thread, THREAD_ERROR, "TPPS:%s pthread_mutex_unlock() : %s", pps_thread->devicename, errno, errbuf);
	}
}
...
#	ifdef __linux__
	...
	if (strncmp(pps_thread->devicename, "/dev/pps", 8) == 0)
	{
		if (pps_check_fake(pps_thread->devicename + 5))
			pps_thread->log_hook(pps_thread,
			                     THREAD_WARN,
			                     "KPPS:%s is fake PPS,"
	...
	else
	{
		...
		if (0 > ioctl(pps_thread->devicefd, TIOCSETD, &ldisc))
		{
			char errbuf[BUFSIZ] = "unknown error";
			pps_strerror_r(errno, errbuf, sizeof(errbuf));
			pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s cannot set PPS line discipline %s", pps_thread->devicename, errbuf);
			return -1;
		}
		...
		for (i = 0; i < globbuf.gl_pathc; i++)
		{
			...
			pps_thread->log_hook(pps_thread, THREAD_PROG, "KPPS:%s checking %s, %s", pps_thread->devicename, globbuf.gl_pathv[i], path);
		...
		if (0 == (int)pps_num)
		{
			pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s device not found.", pps_thread->devicename);
			return -1;
		}
...
#	endif
	...
	/* assert(ret >= 0); */
	pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s RFC2783 path:%s, fd is %d", pps_thread->devicename, path, ret);
	...
	if (0 > time_pps_create(ret, (pps_handle_t*)&inner_context->kernelpps_handle))
	{
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s time_pps_create(%d) failed: %s", pps_thread->devicename, ret, errbuf);
		return -1;
	}
	...
	if (0 > time_pps_getcap(inner_context->kernelpps_handle, &inner_context->pps_caps))
	{
		char errbuf[BUFSIZ]     = "unknown error";
		inner_context->pps_caps = 0;
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s time_pps_getcap() failed: %.100s", pps_thread->devicename, errbuf);
		return -1;
	}
	else
	{
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s pps_caps 0x%02X", pps_thread->devicename, inner_context->pps_caps);
	}
	...
	if (0 == (PPS_TSFMT_TSPEC & inner_context->pps_caps))
	{
		...
		pps_thread->log_hook(pps_thread, THREAD_WARN, "KPPS:%s fail, missing mandatory PPS_TSFMT_TSPEC", pps_thread->devicename);
		return -1;
	}
	if (0 != (PPS_CANWAIT & inner_context->pps_caps))
	{
		/* we can wait! so no need for TIOCMIWAIT */
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s have PPS_CANWAIT", pps_thread->devicename);
	...
	case PPS_CAPTUREASSERT:
		pps_thread->log_hook(
		    pps_thread, THREAD_WARN, "KPPS:%s missing PPS_CAPTURECLEAR, pulse may be offset", pps_thread->devicename);
		pp.mode |= PPS_CAPTUREASSERT;
		break;
	case PPS_CAPTURECLEAR:
		pps_thread->log_hook(
		    pps_thread, THREAD_WARN, "KPPS:%s missing PPS_CAPTUREASSERT, pulse may be offset", pps_thread->devicename);
	...
	default:
		...
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s missing PPS_CAPTUREASSERT and CLEAR", pps_thread->devicename);
		return -1;
	}
	...
	if (0 > time_pps_setparams(inner_context->kernelpps_handle, &pp))
	{
		...
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		pps_thread->log_hook(
		    pps_thread, THREAD_ERROR, "KPPS:%s time_pps_setparams(mode=0x%02X) failed: %s", pps_thread->devicename, pp.mode, errbuf);
 ...
 */
static int
get_edge_tiocmiwait(volatile struct pps_thread_t* thread_context, struct timespec* clock_ts, int* state, volatile struct timedelta_t* last_fixtime)
{
...
#	define PPS_LINE_TIOC (TIOCM_CD | TIOCM_RI | TIOCM_CTS | TIOCM_DSR)
	...
	if (ioctl(thread_context->devicefd, TIOCMIWAIT, PPS_LINE_TIOC) != 0)
	{
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		thread_context->log_hook(
		    thread_context, THREAD_WARN, "TPPS:%s ioctl(TIOCMIWAIT) failed: %d %.40s", thread_context->devicename, errno, errbuf);
		return -1;
		;
	}
	...
	*last_fixtime = thread_context->fix_in;
	...
	if (0 > clock_gettime(CLOCK_REALTIME, clock_ts))
	{
		...
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		thread_context->log_hook(thread_context, THREAD_ERROR, "TPPS:%s clock_gettime() failed: %.100s", thread_context->devicename, errbuf);
		return -1;
		;
	}
	...
	if (ioctl(thread_context->devicefd, (unsigned long)TIOCMGET, state) != 0)
	{
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		thread_context->log_hook(thread_context, THREAD_ERROR, "TPPS:%s ioctl(TIOCMGET) failed: %.100s", thread_context->devicename, errbuf);
		return -1;
	}
	...
	*state &= PPS_LINE_TIOC;
	...
	thread_context->log_hook(thread_context,
	                         THREAD_PROG,
                            ...
                            volatile struct timedelta_t* last_fixtime)
{
	...
	if (0 > time_pps_fetch(inner_context->kernelpps_handle, PPS_TSFMT_TSPEC, &pi, &kernelpps_tv))
	{
		char errbuf[BUFSIZ] = "unknown error";
		pps_strerror_r(errno, errbuf, sizeof(errbuf));
		...
		{
			/* just a timeout */
			thread_context->log_hook(thread_context, THREAD_INF, "KPPS:%s kernel PPS timeout %s", thread_context->devicename, errbuf);
			return 1;
		}
		thread_context->log_hook(thread_context, THREAD_WARN, "KPPS:%s kernel PPS failed %s", thread_context->devicename, errbuf);
		return 0;
	}
...
static void* gpsd_ppsmonitor(void* arg)
{
	...
	if (0 > thread_context->devicefd || 0 == isatty(thread_context->devicefd))
	{
		thread_context->log_hook(thread_context,
		                         THREAD_PROG,
	...
	while (thread_context->report_hook != NULL)
	{
		...
		if (++unchanged == 10)
		{
			...
			unchanged = 0;
			thread_context->log_hook(
			    thread_context, THREAD_WARN, "PPS:%s unchanged state, ppsmonitor sleeps 10", thread_context->devicename);
			(void)sleep(10);
		}
		...
		if (!not_a_tty && !inner_context.pps_canwait)
		{
			...
			ret = get_edge_tiocmiwait(thread_context, &clock_ts_tio, &state_tio, &last_fixtime);
			if (0 != ret)
			{
				thread_context->log_hook(thread_context, THREAD_PROG, "PPS:%s die: TIOCMIWAIT Error", thread_context->devicename);
				break;
			}
			...
			duration_tio = timespec_diff_ns(clock_ts_tio, pulse_tio[edge_tio ? 0 : 1]) / 1000;
		...
		if ((pps_handle_t)0 <= inner_context.kernelpps_handle)
		{
			 ...
			 */
			ret = get_edge_rfc2783(&inner_context, &prev_clock_ts, &prev_edge, &clock_ts_kpps, &edge_kpps, &last_fixtime);
			if (-1 == ret)
			{
				/* error, so break */
				thread_context->log_hook(thread_context, THREAD_ERROR, "PPS:%s die: RFC2783 Error", thread_context->devicename);
				break;
			}
			...
			thread_context->log_hook(thread_context,
			                         ...
			                         thread_context->devicename,
			                         edge_str,
			                         (long long)cycle_kpps,
			                         (long long)duration_kpps,
		...
		if (not_a_tty && !inner_context.pps_canwait)
		{
			...
			thread_context->log_hook(
			    thread_context, THREAD_WARN, "PPS:%s die: no TIOMCIWAIT, nor RFC2783 CANWAIT", thread_context->devicename);
			break;
		}
		/*
		 * End of Stage One
		...
		{
			thread_context->log_hook(
			    thread_context, THREAD_RAW, "PPS:%s %.10s pps-detect changed to %d", thread_context->devicename, edge_str, state);
			unchanged = 0;
		}
		...
		{ /* 0.5Hz */
			...
			unchanged = 0;
			thread_context->log_hook(
			    thread_context, THREAD_RAW, "PPS:%s %.10s pps-detect invisible pulse", thread_context->devicename, edge_str);
		}
		...
		if (!ok)
		{
			...
			thread_context->log_hook(thread_context, THREAD_PROG, "PPS:%s %.10s ignored %.100s", thread_context->devicename, edge_str, log);
			continue;
		}
		...
		thread_context->log_hook(thread_context, THREAD_RAW, "PPS:%s %.10s categorized %.100s", thread_context->devicename, edge_str, log);
		...
		{
			thread_context->log_hook(
			    thread_context, THREAD_WARN, "PPS:%s %.10s system clock went backwards: %.20s", thread_context->devicename, edge_str, delay_str);
			log1 = "system clock went backwards";
		}
		else if ((2 < delay.tv_sec) || (1 == delay.tv_sec && 100000000 < delay.tv_nsec))
		{
			/* system clock could be slewing so allow up to 1.1 sec delay */
			/* chronyd can slew +/-8.33% */
			thread_context->log_hook(
			    thread_context, THREAD_WARN, "PPS:%s %.10s no current GPS seconds: %.20s", thread_context->devicename, edge_str, delay_str);
	...
	{
		thread_context->log_hook(thread_context, THREAD_PROG, "KPPS:%s descriptor cleaned up", thread_context->devicename);
		(void)time_pps_destroy(inner_context.kernelpps_handle);
	}
#endif
	thread_context->log_hook(thread_context, THREAD_PROG, "PPS:%s gpsd_ppsmonitor exited.", thread_context->devicename);
	return NULL;
}
...
void pps_thread_activate(volatile struct pps_thread_t* pps_thread)
{
	...
	if ((pps_handle_t)0 <= inner_context.kernelpps_handle)
	{
		pps_thread->log_hook(pps_thread, THREAD_INF, "KPPS:%s kernel PPS will be used", pps_thread->devicename);
	}
	else
	{
		pps_thread->log_hook(pps_thread,
...
#else
	pps_thread->log_hook(pps_thread, THREAD_WARN, "KPPS:%s no HAVE_SYS_TIMEPPS_H, PPS accuracy will suffer", pps_thread->devicename);
#endif

	memset(&pt, 0, sizeof(pt));
	retval = pthread_create(&pt, NULL, gpsd_ppsmonitor, (void*)&inner_context);
	pps_thread->log_hook(pps_thread, THREAD_PROG, "PPS:%s thread %s", pps_thread->devicename, (retval == 0) ? "launched" : "FAILED");
...
void pps_thread_fixin(volatile struct pps_thread_t* pps_thread, volatile struct timedelta_t* fix_in)
{
...
void pps_thread_qErrin(volatile struct pps_thread_t* pps_thread, long qErr, struct timespec qErr_time)
{
	thread_lock(pps_thread);
...
int pps_thread_ppsout(volatile struct pps_thread_t* pps_thread, volatile struct timedelta_t* td)
{
	volatile int ret;
