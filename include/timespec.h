...
#	define TSTOMS(ts)             ((int64_t)((ts)->tv_sec) * MS_IN_SEC + (int64_t)((ts)->tv_nsec / MS_IN_NS))
...
#	define timespec_diff_ns(x, y) (int64_t)((((x).tv_sec - (y).tv_sec) * NS_IN_SEC) + (x).tv_nsec - (y).tv_nsec)
...
#	define TS_SUB_D(ts1, ts2) \
		((double)((ts1)->tv_sec - (ts2)->tv_sec) + ((double)((ts1)->tv_nsec - (ts2)->tv_nsec) * 1e-9))
...
#	define TS_GE(ts1, ts2) \
		((ts1)->tv_sec > (ts2)->tv_sec || ((ts1)->tv_sec == (ts2)->tv_sec && (ts1)->tv_nsec >= (ts2)->tv_nsec))

// true if normalized timespec1 equal to timespec2
#	define TS_EQ(ts1, ts2) ((ts1)->tv_sec == (ts2)->tv_sec && (ts1)->tv_nsec == (ts2)->tv_nsec)
